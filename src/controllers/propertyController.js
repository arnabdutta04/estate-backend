const { Property, Broker, User, Schedule } = require('../models');
...
exports.getAllProperties = async (req, res) => {
  try {
    ...
    if (listingType || propertyFor) whereConditions.listingType = listingType || propertyFor;
    if (style) whereConditions.style = style;

    if (minPrice || maxPrice) {
      whereConditions.price = {};
      if (minPrice && !isNaN(parseFloat(minPrice))) whereConditions.price[Op.gte] = parseFloat(minPrice);
      if (maxPrice && !isNaN(parseFloat(maxPrice))) whereConditions.price[Op.lte] = parseFloat(maxPrice);
    }
    ...
    if (bedrooms && parseInt(bedrooms) > 0) {
      whereConditions.bedrooms = { [Op.gte]: parseInt(bedrooms) };
    }

    if (bathrooms && parseInt(bathrooms) > 0) {
      whereConditions.bathrooms = { [Op.gte]: parseInt(bathrooms) };
    }

    if (diningRooms && parseInt(diningRooms) > 0) {
      whereConditions.diningRooms = { [Op.gte]: parseInt(diningRooms) };
    }
    ...
    if (facilities) {
      ...
      facilityArray.forEach(facility => {
        ...
        if (dbColumn === 'furnished') {
          whereConditions.furnished = { [Op.in]: ['furnished', 'semi-furnished'] };
        } else {
...
exports.getPropertyById = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, property: transformedProperty });
...
exports.createProperty = async (req, res) => {
  try {
    const {
      title, description, propertyType, listingType, propertyFor,
      price, address, city, state, pincode, country, latitude, longitude,
      bedrooms, bathrooms, diningRooms, area, furnished,
      parkingSlot, wifi, security, kitchen, ac, swimmingPool, gym,
      petAllowed, homeTheater, spa, elevator, conferenceRoom,
      gatedCommunity, waterSupply, electricity,
      images, yearBuilt, age, condition, style, metaTitle, metaDescription
    } = req.body;
...
// @desc    Get broker's own properties
...
exports.getMyProperties = async (req, res) => {
  try {
    ...
    const transformedProperties = properties.map(property => ({
      ...
      propertyFor: property.listingType,
      price: property.price,
      location: { city: property.city, state: property.state, address: property.address },
      city: property.city,
      specifications: {
        bedrooms: property.bedrooms || 0,
        bathrooms: property.bathrooms || 0,
...
exports.toggleFeatured = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error updating featured status' });
  }
};
...
module.exports = {
  ...
  scheduleVisit: exports.scheduleVisit,
